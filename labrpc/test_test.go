...
func TestBasic(t *testing.T) {
	...
	raftSrv := raft.NewServer(nil)
	svc := MakeService("RaftServer", raftSrv)
	...
	{
		reply, err := e.Call("RaftServer.RequestVote", context.Background(), &pb.RequestVoteRequest{})
		assert.Equal(t, nil, err)
		_ = reply.(*pb.RequestVoteReply)
	}

	{
		reply, err := e.Call("RaftServer.AppendEntries", context.Background(), &pb.AppendEntriesRequest{})
		assert.Equal(t, nil, err)
		_ = reply.(*pb.AppendEntriesReply)
	}
}
