...
import (
	...
	"github.com/singlemonad/SnailKV/shardmaster/internal/app"
...
type config struct {
	...
	clerks       map[*shardmaster.Clerk][]string
	nextClientId int
}
...
func (cfg *config) makeClient(to []int) *shardmaster.Clerk {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
	...
	smClients := make([]pb.ShardMasterClient, 0)
	for _, end := range ends {
		smClients = append(smClients, end)
	}

	ck := shardmaster.MakeClerk(smClients)
...
func (cfg *config) deleteClient(ck *shardmaster.Clerk) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
...
func (cfg *config) ConnectClientUnlocked(ck *shardmaster.Clerk, to []int) {
...
func (cfg *config) ConnectClient(ck *shardmaster.Clerk, to []int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
	cfg.ConnectClientUnlocked(ck, to)
}
...
// caller should hold cfg.mu
func (cfg *config) DisconnectClientUnlocked(ck *shardmaster.Clerk, from []int) {
...
func (cfg *config) DisconnectClient(ck *shardmaster.Clerk, from []int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
...
func (cfg *config) StartServer(i int) {
	...
	// init raft peer clients
	raftClients := make([]pb.RaftClient, 0)
	for _, end := range ends {
		raftClients = append(raftClients, end)
	}
	...
	var cfgPath string
	if cfg.n == 4 {
		cfgPath = fmt.Sprintf("./3Server/config-%d.json", i)
	} else {
		cfgPath = fmt.Sprintf("./5Server/config-%d.json", i)
	}
	cfg.servers[i] = app.NewFakeServer(cfgPath, raftClients)

	kvsvc := labrpc.MakeService("ShardMasterServer", cfg.servers[i])
	rfsvc := labrpc.MakeService("RaftServer", cfg.servers[i].GetRaftServer())
...
func make_config(t *testing.T, n int, unreliable bool) *config {
	...
	cfg.n = n
	cfg.servers = make([]*app.Server, cfg.n)
	cfg.saved = make([]*raft.Persister, cfg.n)
	cfg.endnames = make([][]string, cfg.n)
	cfg.clerks = make(map[*shardmaster.Clerk][]string)
